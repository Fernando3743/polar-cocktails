...
export function ComboForm({ comboId, initial }: ComboFormProps) {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const parsed = comboSchema.safeParse(payload);
    if (!parsed.success) {
    ...
    try {
      const result = isEdit
        ? await updateCombo(comboId as string, parsed.data)
        : await createCombo(parsed.data);
      ...
      setFormError("No pudimos guardar el combo. Inténtalo de nuevo.");
      setSubmitting(false);
    }
  }
      ...
      <div className="glass-card flex flex-col gap-5 p-6">
        ...
        <Field label="Descripción (opcional)" error={errors.description}>
          <textarea
            ...
            placeholder="Cuatro granizados + una botella para la fiesta."
          />
        </Field>
        ...
        <Field label="Color de acento" error={errors.accentColor}>
          <div className="flex items-center gap-3">
            <input
              type="color"
              value={validColor ? accentColor : "#9128da"}
      ...
      <aside className="mx-auto w-full max-w-[360px] lg:max-w-none">
        <div className="glass-card flex flex-col items-center gap-4 p-6 lg:sticky lg:top-[104px]">
          ...
          <div className="w-full text-center">
            ...
            <p className="mt-1 text-sm font-bold text-polar-magenta">
              {formatCop(Number.parseInt(priceCop, 10) || 0)}
            </p>
          </div>
        </div>
      </aside>
